...
让 agent 感受到两次对话之间的"沉默"。每次 run 开始前，算出距上次活动的间隔，按自然语言分桶（`[距上次说话 3 小时]`、`[3 天没说话了]`、`[刚聊过没多久]` 等）prepend 到首条用户消息；<2 分钟视为连续对话不加 hint。分桶措辞统一使用"距上次说话/没说话了"的句式避免被 LLM 误读成时间戳。hint 只进当前 run 的 LLM 输入，不写入历史。
